...
def only_one_cluster_sample():
    sample = multivariate_normal(mean=(.5, .5), cov=1.0 / 200).rvs(500)
    return sample
...
def test_no_clusters_asc(uniform_sample):
    shdbscan = SHDBSCAN(min_cluster_size=100, allow_single_cluster=True).fit(uniform_sample)
...
def test_one_cluster_no_noise_asc(only_one_cluster_sample):
    shdbscan = SHDBSCAN(min_cluster_size=500, allow_single_cluster=True).fit(only_one_cluster_sample)
    assert shdbscan.n_classes == 1
...
def test_one_cluster_no_noise_aasc(only_one_cluster_sample):
    shdbscan = SHDBSCAN(min_cluster_size=500, auto_allow_single_cluster=True).fit(only_one_cluster_sample)
    assert shdbscan.n_classes == 1
    assert np.all(shdbscan.labels == 0)
    assert shdbscan.proba.shape == (only_one_cluster_sample.shape[0], 1)
...
def test_one_cluster_with_noise_asc(one_cluster_sample):
    shdbscan = SHDBSCAN(min_cluster_size=500, allow_single_cluster=True).fit(one_cluster_sample)
    assert shdbscan.n_classes == 2
    assert shdbscan.proba.shape == (one_cluster_sample.shape[0], 2)

#       II. by aasc -> finds 1
def test_one_cluster_with_noise_aasc(one_cluster_sample):
    shdbscan = SHDBSCAN(min_cluster_size=500, auto_allow_single_cluster=True).fit(one_cluster_sample)
    assert shdbscan.n_classes == 2
    assert shdbscan.proba.shape == (one_cluster_sample.shape[0], 2)
...
def test_two_clusters_asc(two_clusters_sample):
    shdbscan = SHDBSCAN(min_cluster_size=250, allow_single_cluster=True).fit(two_clusters_sample)
    assert shdbscan.n_classes == 2
    assert shdbscan.proba.shape == (two_clusters_sample.shape[0], 2)

#       II. by aasc -> finds 2
def test_two_clusters_aasc(two_clusters_sample):
    shdbscan = SHDBSCAN(min_cluster_size=250, auto_allow_single_cluster=True).fit(two_clusters_sample)
    assert shdbscan.n_classes == 3
    assert shdbscan.proba.shape == (two_clusters_sample.shape[0], 3)

#   d. not forced with 1 center -> finds 1
def test_two_clusters_one_center(two_clusters_sample):
    shdbscan = SHDBSCAN(min_cluster_size=250, allow_single_cluster=True).fit(two_clusters_sample, centers=(.25, .25))
    assert shdbscan.n_classes == 2
    assert shdbscan.proba.shape == (two_clusters_sample.shape[0], 2)
    # found correct one
    center = two_clusters_sample[shdbscan.labels == 0].mean(axis=0)
    assert squarediff(center, (.25, .25)) < squarediff(center, (.75, .75))
...
def test_two_clusters_two_centers(two_clusters_sample):
    ...
    shdbscan = SHDBSCAN(min_cluster_size=250, allow_single_cluster=True).fit(two_clusters_sample, centers=[(30, 30),(500, 500)])
    assert shdbscan.n_classes == 3
    assert shdbscan.proba.shape == (two_clusters_sample.shape[0], 3)
...
def test_three_clusters_two_centers(three_clusters_sample):
    shdbscan = SHDBSCAN(min_cluster_size=90, auto_allow_single_cluster=True).fit(
        three_clusters_sample, centers=[(.75, .75),(.25, .25)])
    assert shdbscan.n_classes == 3
    ...
    assert squarediff(center, (.75, .75)) < squarediff(center, (.25, .25))
    center2 = three_clusters_sample[shdbscan.labels == 1].mean(axis=0)
    assert squarediff(center2, (.25, .25)) < squarediff(center2, (.75, .75))
