...
class TestHelpers:
    def test_coord_transform(self):
        cartesian = np.random.uniform(
            low=-16204.0, high=16204.0, size=(1000, 3)
        )
    ...
    def test_uniform_circle(self):
        ...
        assert (
            kstest(sx, "uniform", args=(sx.min(), sx.max() - sx.min())).pvalue
            > 0.05
        )
        assert (
            kstest(sy, "uniform", args=(sy.min(), sy.max() - sy.min())).pvalue
            > 0.05
        )
    ...
    def test_uniform_sphere(self):
        ...
        assert (
            data[np.sqrt(dx ** 2 + dy ** 2 + dz ** 2) > radius].shape[0] == 0
        )
        assert data[~is_inside_circle(center, radius, data)].shape[0] == 0
        ...
        assert (
            kstest(sz, "uniform", args=(sz.min(), sz.max() - sz.min())).pvalue
            > 0.05
        )
