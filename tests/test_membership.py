...
from opencluster.synthetic import Field, Cluster, Synthetic, BivariateUnifom, UniformFrustum, polar_to_cartesian
...
def sample1c(fmix=.9):
    ...
    clusters = [
        Cluster(
            ...
            pm=multivariate_normal(mean=(-5.75, 7.25), cov=1./34),
...
def sample2c(fmix=.6):
    ...
    clusters = [
        Cluster(
            space=multivariate_normal(mean=polar_to_cartesian([120.5, -27.25, 1.57]), cov=50),
            pm=multivariate_normal(mean=(-5.4, 6.75), cov=1./34),
            star_count=int(n*cmix),
        ),
        Cluster(
            space=multivariate_normal(mean=polar_to_cartesian([121.75, -28.75, 1.63]), cov=50),
            pm=multivariate_normal(mean=(-6.25, 7.75), cov=1./34),
            star_count=int(n*cmix),
        ),
    ]
    return Synthetic(field=field, clusters=clusters).rvs()
...
def test_sample1c(sample1c):
    ...
    assert mean_sqdiff(proba, res) < .01
...
def test_sample2c(sample2c):
    ...
    labels = get_labels(sample2c[['p_pm_field', 'p_pm_cluster1', 'p_pm_cluster2']].values)
    init_proba = one_hot_encode(labels)
    dbme = DBME().fit(data=data, init_proba=init_proba)
    assert dbme.posteriors.shape == init_proba.shape
    ...
    assert mean_sqdiff(proba1, res1) < .01
    assert mean_sqdiff(proba2, init_proba[:,2]) > mean_sqdiff(proba2, res2)
    assert mean_sqdiff(proba2, res2) < .01
