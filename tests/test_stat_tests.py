...
from opencluster.stat_tests import DipDistTest, HopkinsTest, RipleysKTest
...
@pytest.mark.parametrize("mode, pvalue, passed", [
    ('ripley', .05, True),('ripley', .01, False),('chiu', .1, True),('chiu', .05, True),('chiu', .01, False)])
def test_ripleysk_empirical_rule(mode, pvalue, passed):
    radii = np.array([1,2,3])
    lf = np.array([1.01, 2.01, 3.02])
    area = 1.5
    n = 100
    value1, passed1 = RipleysKTest(pvalue_threshold=pvalue, mode=mode).empirical_csr_rule(radii=radii, l_function=lf, area=area, n=n)
...
def test_ripleysk_uniform(uniform_sample):
    ...
    assert not RipleysKTest(mode='chiu', pvalue_threshold=.01).test(data=uniform_sample).passed
...
def test_ripleysk_iris(iris):
    ...
    with pytest.warns(UserWarning) as record:
        rk = RipleysKTest(pvalue_threshold=.01).test(iris[:, :2])
        assert rk.passed
        assert np.allclose(rk.radii, spatstat_result.radius.to_numpy())
        # difference due to astropy implementation
        assert np.allclose(rk.l_function, spatstat_result.l_function.to_numpy(), atol=1e-2)
        assert np.isclose(spatstat_value, rk.value, atol=1e-2)
    record[0].message.args[0] == "There are repeated data points that cause astropy.stats.RipleysKEstimator to break, they will be removed."
