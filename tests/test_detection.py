...
from opencluster.detection2 import (
    get_histogram_bins, histogram, get_default_mask, fast_std_filter, nyquist_offsets, Peak, get_most_significant_peaks,
    CountPeakDetector,
)
import pandas as pd
import math
...
def test_get_histogram_bins():
    ...
    bins, ranges = get_histogram_bins(data, [.5, .5])
    assert np.allclose(ranges, np.array([[-.05, .95], [-.05, .95]]))
    assert np.allclose(bins, np.array([2, 2]))
    # data has range 0 to 1
    data2 = np.vstack((np.arange(0, 1.1, 0.1), np.arange(0, 1.1, 0.1))).T
    bins, ranges = get_histogram_bins(data2, [.5, .5])
    assert np.allclose(ranges, np.array([[-.25, 1.25], [-.25, 1.25]]))
...
def test_get_histogram_bins_offset():
    ...
    bins, ranges = get_histogram_bins(data, [.5, .5], [-.31, .17])
    assert np.allclose(ranges, np.array([[-.56, .94], [-.08, 1.42]]))
    assert np.allclose(bins, np.array([3, 3]))

def test_histogram():
    correct_edges = np.array([[-.25, .25, .75, 1.25], [-.25, .25, .75, 1.25]])
    data = np.vstack((np.arange(0, 1.1, 0.1), np.arange(0, 1.1, 0.1))).T
    # should be 3 by 3 histogram, showing the linear data distribution
    hist, edges = histogram(data, [.5, .5])
    ...
    hist, edges = histogram(data, [.5, .5], [.5, .5])
    assert np.allclose(hist, np.array([[5, 0, 0], [0, 3, 0], [0, 0, 0]]))
    assert np.allclose(edges, correct_edges + .5)
...
def test_fast_std_filter_with_mask():
    ...
    assert np.allclose(result, np.array([
       [0.       , 0.       , 0.2763854, 0.       , 0.       ],
       [0.       , 0.2763854, 0.2763854, 0.2763854, 0.       ],
       [0.2763854, 0.2763854, 0.       , 0.2763854, 0.2763854],
       [0.       , 0.2763854, 0.2763854, 0.2763854, 0.       ],
       [0.       , 0.       , 0.2763854, 0.       , 0.       ],
       ]))
...
@pytest.mark.parametrize("peaks, expected", [
    ([Peak(np.array([0, 0]), significance=3), Peak(np.array([0, 1]), significance=2), Peak(np.array([1,2]), significance=6), Peak(np.array([2, 2]), significance=7)], [[0, 0], [2, 2]]),
    ([Peak(np.array([0, 1]), significance=3), Peak(np.array([0, 2]), significance=3)], [[0,1]]),
    ([], []),
])
...
def test_count_based_outlier_removal_high_min_count():
    ...
    with pytest.raises(ValueError):
        CountPeakDetector(bin_shape=[1,1], mask=mask, min_count=6).trim_low_density_regions(data)

@pytest.mark.parametrize("data, mask, bin_shape", [
    (np.zeros(10), [1,1], [1,1]),
...
def test_detect_warnings():
    with pytest.warns(UserWarning):
        CountPeakDetector(bin_shape=[1,5], mask=np.ones((3,3))/9).detect(diagonal1_data())
...
def test_detect_happy_pass(three_clusters_sample):
    ...
    data = df.values
    result = CountPeakDetector(bin_shape=[.5, .5, .05], min_count=5, min_dif=20).detect(data)
    center1 = (7.5, 7, np.log10(5))
    center2 = (4.5, 4, np.log10(5))
    center3 = (.5, 0, np.log10(5))
