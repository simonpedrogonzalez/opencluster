...
def get_default_mask(dim: int):
    indexes = np.array(
        np.meshgrid(*np.tile(np.arange(5), (dim, 1)))
    ).T.reshape((-1, dim))
    mask = np.zeros([5] * dim)
    cond = np.sum((indexes - 2) ** 2, axis=1)
    mask[
        tuple(
            indexes[np.argwhere((cond > 0) & (cond < 5))].reshape((-1, dim)).T
        )
    ] = 1
...
def convolve(
    data, mask: np.ndarray = None, c_filter: callable = None, *args, **kwargs
):
    if c_filter:
...
def get_histogram_bins(
    data: np.ndarray, bin_shape: np.ndarray, offsets: list = None
):
    _, dim = data.shape
    ...
    if offsets is not None:
        ranges = [
            [ranges[i][0] + offsets[i], ranges[i][1] + offsets[i]]
            for i in range(dim)
        ]
    bins = [
        round((ranges[i][1] - ranges[i][0]) / bin_shape[i]) for i in range(dim)
    ]
        ...
        for i in range(dim):
            ...
            for j in range(2):
                while True:
                    ...
                    slice_ranges[i][int(not (j))] = (
                        slice_ranges[i][j] + shifts[j]
                    )
                    data_slice = data[
                        RangeMasker(limits=slice_ranges).mask(data)
                    ]
                    if data_slice.shape[0] != 0:
                        slice_histogram, _ = histogram(
                            data_slice, self.bin_shape
                        )
    ...
    def detect(self, data: np.ndarray, heatmaps:bool=False):
        ...
        for offset in self.offsets:
            ...
            clusters_idx = peak_local_max(
                normalized, **peak_detection_params
            ).T
