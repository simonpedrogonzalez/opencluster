...
class UniformFrustum(stats._multivariate.multi_rv_frozen):
    ...
    def is_inside_shape(self, data):
        ...
        mask[(polar[:, 0] > self.locs[0]) & (polar[:, 0] < self.locs[0]+self.scales[0]) &\
        (polar[:, 1] > self.locs[1]) & (polar[:, 1] < self.locs[1]+self.scales[1]) &\
        (polar[:, 2] > self.locs[2]) & (polar[:, 2] < self.locs[2]+self.scales[2])] = 1
    ...
    def volume(self):
        vertices = np.array([
            ...
            (self.locs[0]+self.scales[0], self.locs[1]+self.scales[1], self.locs[2]),
            (self.locs[0], self.locs[1], self.locs[2]+self.scales[2]),
            (self.locs[0]+self.scales[0], self.locs[1], self.locs[2]+self.scales[2]),
            (self.locs[0], self.locs[1]+self.scales[1], self.locs[2]+self.scales[2]),
            (self.locs[0]+self.scales[0], self.locs[1]+self.scales[1], self.locs[2]+self.scales[2]),
        ])
    ...
    def rvs(self, size=1):
        extremes = np.array([
            ...
            (self.locs[0], self.locs[1]+self.scales[1], self.locs[2]+self.scales[2]),
            (self.locs[0]+self.scales[0], self.locs[1]+self.scales[1], self.locs[2]+self.scales[2]),
            (self.locs[0] + self.scales[0]/2, self.locs[1] + self.scales[1]/2, self.locs[2]),
            (self.locs[0] + self.scales[0]/2, self.locs[1] + self.scales[1]/2, self.locs[2]+self.scales[2]),
        ])
...
def one_cluster_sample_small(field_size=int(1e3), cluster_size=int(2e2)):
    ...
    clusters = [
        Cluster(
            space=stats.multivariate_normal(mean=polar_to_cartesian([120.7, -28.5, 5]), cov=1.5),
            pm=stats.multivariate_normal(mean=(.5, 0), cov=1./35),
            star_count=cluster_size
        ),
    ]
...
def case1_sample1c(fmix=.6):
    ...
    clusters = [
        Cluster(
            space=stats.multivariate_normal(mean=polar_to_cartesian([121, -28, 1.6]), cov=7),
            pm=stats.multivariate_normal(mean=(-5.75, 7.25), cov=1./34),
            star_count=int(n*cmix),
        ),
    ]
    return Synthetic(field=field, clusters=clusters).rvs()

def case1_sample2c(fmix=.6):
    n = 1000
    n_clusters = 2
    cmix = (1-fmix)/n_clusters
    field = Field(
    ...
    clusters = [
        Cluster(
            space=stats.multivariate_normal(mean=polar_to_cartesian([120.5, -27.25, 1.57]), cov=7),
            pm=stats.multivariate_normal(mean=(-5.4, 6.75), cov=1./34),
            star_count=int(n*cmix),
        ),
        Cluster(
            space=stats.multivariate_normal(mean=polar_to_cartesian([121.75, -28.75, 1.63]), cov=7),
...
def case2_sample0c(fmix=None):
    ...
    field = Field(
        pm=BivariateUnifom(locs=(-7, 6), scales=(2.5, 2.5)),
        space=UniformFrustum(locs=(118, -31, 1.2), scales=(6, 6, .9)),
        star_count=int(n),
        )
    return Synthetic(field=field, clusters=[]).rvs()

def case2_sample1c(fmix=.6):
    n = 1000
    n_clusters = 1
    cmix = (1-fmix)/n_clusters
    ...
    clusters = [
        Cluster(
            space=stats.multivariate_normal(mean=polar_to_cartesian([121, -28, 1.6]), cov=50),
            pm=stats.multivariate_normal(mean=(-5.75, 7.25), cov=1./34),
            star_count=int(n*cmix),
        ),
    ]
