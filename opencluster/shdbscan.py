...
class SHDBSCAN():
    ...
    outlier_quantile:float = field(default=None, validator=validators.optional([validators.ge(0), validators.le(1)]))

    @min_cluster_size.validator
    def min_cluster_size_validator(self, attribute, value):
        if value is None:
            if self.clusterer is None:
                raise ValueError("either min_cluster_size or clusterer must be provided")
        elif value < 1:
    ...
    def get_proba(self):
        ...
        if not np.any(self.unique_labels == -1):
            one_hot_code = np.vstack((np.zeros(one_hot_code.shape[0]), one_hot_code.T)).T
        ...
        if self.outlier_quantile is not None:
            ...
            outlier_scores = MinMaxScaler().fit_transform(outlier_scores.reshape(-1, 1)).ravel()
        ...
        if self.clusterer.allow_single_cluster:
            ...
            # 1/0
            cluster_proba = one_hot_code[:, 1:] * \
                np.tile((1-noise_proba), (n_clusters, 1)).T
    ...
    def center_based_cluster_selection(self, data, labels, input_centers):
        ...
        if short.shape[0] == 1:
            ...
            dist_idcs = tuple([tuple(map(tuple, x)) for x in np.stack(
                (np.tile(idx_rows, (idx_columns.shape[0], 1)), idx_columns), axis=1)])
            distance_sum_per_solution = np.array(
                [center_distances[dist_idcs[i]] for i in range(len(dist_idcs))]).sum(axis=1)

        best_solution = idx_columns[distance_sum_per_solution ==
                                    distance_sum_per_solution.min()].ravel()
        ...
        noise_proba = posteriors[:, tuple(
            [0] + list((cluster_labels + 1)[~np.isin(cluster_labels, best_solution)]))].sum(axis=1)
        cluster_proba = posteriors[:, tuple(
            (cluster_labels + 1)[np.isin(cluster_labels, best_solution)])]
    ...
    def fit(self, data: np.ndarray, centers: np.ndarray = None):
        ...
        if not self.clusterer.allow_single_cluster and self.centers_provided and self.centers.shape[0] < n_clusters:
    ...
    def memplot(self, label=0, **kwargs):
        return membership_plot(data=self.data, posteriors=self.proba[:,label+1], labels=self.labels, **kwargs)

    def plot3d(self, label=0, **kwargs):
        return membership_3d_plot(self.data, self.proba[:,label+1], self.labels + 1, **kwargs)

    def plot_out_dist(self, **kwargs):
        outlier_scores = self.clusterer.outlier_scores_[np.isfinite(self.clusterer.outlier_scores_)]
        ax = sns.distplot(
            outlier_scores,
            rug=True, kde_kws={"cut": 0}, **kwargs)
        return ax
...
def one_cluster_sample():
    ...
    sample2 = multivariate_normal(mean=(.5, .5), cov=1.0 / 200).rvs(100)
    return np.concatenate((sample, sample2))

def two_clusters_sample():
    sample = BivariateUnifom(locs=(0, 0), scales=(1, 1)).rvs(800)
    sample2 = multivariate_normal(mean=(.75, .75), cov=1.0 / 200).rvs(100)
    sample3 = multivariate_normal(mean=(.25, .25), cov=1.0 / 200).rvs(100)
...
def test_0c_force():
    ...
    data = uniform_sample()
    s = SHDBSCAN(min_cluster_size=100, auto_allow_single_cluster=True).fit(data) # or allow_single_cluster=True
    s.plot()
    plt.show()
    print('coso')
...
def test_1c_force():
    # cluster is found because it is forced
    data = one_cluster_sample()
    s = SHDBSCAN(min_cluster_size=100, auto_allow_single_cluster=True, outlier_quantile=.9).fit(data)
    s.plot()
    plt.show()
...
def test_2c_center():
    ...
    data = two_clusters_sample()
    s = SHDBSCAN(min_cluster_size=100, auto_allow_single_cluster=True, outlier_quantile=.9).fit(data, centers=np.array(((.25, .25))))
    s.plot()
    plt.show()
    print('coso')
...
def test_no_clusters_aasc(uniform_sample):
    shdbscan = SHDBSCAN(min_cluster_size=100, auto_allow_single_cluster=True).fit(uniform_sample)
...
def three_clusters_sample():
    ...
    sample2 = multivariate_normal(mean=(.75, .75), cov=1.0 / 200).rvs(160)
    sample3 = multivariate_normal(mean=(.25, .25), cov=1.0 / 200).rvs(160)
    sample4 = multivariate_normal(mean=(.5, .5), cov=1.0 / 200).rvs(160)
...
def test_three_clusters_two_centers(three_clusters_sample):
    shdbscan = SHDBSCAN(min_cluster_size=80, auto_allow_single_cluster=True).fit(
        three_clusters_sample, centers=[(.75, .75),(.5, .5)])
    ...
    assert center - np.array([.25, .25]) < center - np.array([.75, .75])
    center2 = three_clusters_sample[shdbscan.labels == 1].mean(axis=0)
    assert center2 - np.array([.75, .75]) < center2 - np.array([.25, .25])
